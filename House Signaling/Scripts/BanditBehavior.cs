...
{   
    ...
    private void Update()
    {
        if (_isStopped == false)
        {
            ...
            Vector3 targetPosition = _waypoints[_waypointIndex].GetCoordinate();
            transform.LookAt(targetPosition);
            transform.position = Vector3.MoveTowards(currentPosition, targetPosition, _waypoints[_waypointIndex].GetSpeed() * Time.deltaTime);
   ...
   {
        if (_waypoints[index].CheckPause() == true)
            StartCoroutine(RunPause(_waypoints[index].GetPauseTime()));
   }
